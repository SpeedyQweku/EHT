...
def parse_args():
    ...
    parser.add_argument(
        "-d", "--domain", dest="domain", type=str, required=True, help="target domain."
    )
    ...
    try:
        crt_result, status_code = main(arg.domain)
        ...
        if status_code != 200:
            ...
            if arg.domain and arg.output and not arg.wildcard and not arg.notwildcard:
                for subdomain in result:
            ...
            if arg.domain and arg.wildcard and not arg.output and not arg.notwildcard:
                for subdomain in result:
                    if subdomain.startswith("*."):
                        print(subdomain.strip())

            # For all the subdomains without the wildcard subdomains
            if arg.domain and arg.notwildcard and not arg.output and not arg.wildcard:
                for subdomain in result:
                    if not subdomain.startswith("*."):
                        print(subdomain.strip())
